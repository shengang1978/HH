...
//SR FS Needs Review [JAW  20040912] - inline asm looks risky, adding WinCPUDetectGetCPUCaps to list of risky routines
//SR FS: Reviewed [wwt 20040915]: safe input param; no MBCS; no dangerous API; no registry/sys folder/temp file 
//SR FS: Reviewed [HBY 20041018]: no risk
#if defined (_WIN32)

#include <cstring>
#include "WinCPUDetect.h"
...
/*
 Routine:   WinCPUDetectGetCPUCaps
 ...
 Comment:   This function returns information about the capabilies of the
            ...
            use WinCPUDetectGetCPUCaps() inside time-critical code.

*/
ULONG_PTR WinCPUDetectGetCPUCaps (CPUCAPS cap)
{
    static int init = 0;
    ...
    case CPU_VENDOR_STRING:
        res = reinterpret_cast<ULONG_PTR>(proc_idstr);
        break;
    ...
    case CPU_NAME_STRING:
        res = reinterpret_cast<ULONG_PTR>(proc_namestr);
        break;
    ...
    case CPU_TYPE:
        ...
        // as MMX and 3DNow!  Instead, use WinCPUDetectGetCPUCaps (HAS_MMX) and WinCPUDetectGetCPUCaps (HAS_3DNOW),
        // which will accurately detect the presence of these features on all chips which
        // support them.
        res = UNKNOWN;
        switch (WinCPUDetectGetCPUCaps (CPU_VENDOR))
        {
        case VENDOR_AMD:
...
// **CodeWizzard** - Violation:  Universal Coding Standards item 35  - Always provide a default branch for switch statement
    ...
    case HAS_SSE_FP:
    	...
    	if (WinCPUDetectGetCPUCaps (HAS_SSE))
    	{
	    	__try
	    	{
...
#else if
      #pragma message("No pure c/c++ implement!")
#endif
				res = 1;
	    	}
	    	__except (1)
    ...
    default:
        ...
        if (WinCPUDetectGetCPUCaps (CPU_VENDOR) == VENDOR_AMD)
        {
...
// **CodeWizzard** - Violation:  Universal Coding Standards item 35  - Always provide a default branch for switch statement
            ...
            if (WinCPUDetectGetCPUCaps (CPU_TYPE) == AMD_ATHLON)
            {
...
/*
 Routine:   detect_base ()
 Comment:   This routine is separate from WinCPUDetectGetCPUCaps() for ease of
...
#elif (!defined (__unix__) && !defined COMPILE_64_BIT)
...
foundit:
      ...
      #pragma message("No pure c/c++ implement!")
#endif
#if(!defined __unix__ && !defined COMPILE_64_BIT)
    __asm
    {
