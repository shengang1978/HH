(function() {
		...
		if ((k = j.match(o))) {
			...
			if (Number(f) < 1000) {
				...
				for (g = 1; g * 3 < f.length; g++) {
					n = b
							+ f.substring(f.length - g * 3, f.length - (g - 1)
									* 3) + n
				}
	...
	this.jSliderIsArray = function(a) {
		...
		if (a instanceof Array
				|| (!(a instanceof Object)
						&& (Object.prototype.toString.call((a)) == "[object Array]") || typeof a.length == "number"
...
(function() {
	...
	this.jSliderTmpl = function b(e, d) {
		var c = !(/\W/).test(e) ? a[e] = a[e] || b(e) : new Function("obj",
...
(function(a) {
	...
	Draggable.prototype = {
		...
		_events : function() {
			var b = this;
			this.supportTouches_ = (a.browser.webkit && navigator.userAgent
					.indexOf("Mobile") != -1);
			this.events_ = {
		...
		_mousedown : function(b) {
			...
			if (this.outer && this.outer.get(0)) {
				this.outer.css( {
					height : Math.max(this.outer.height(), a(document.body)
							.height()),
					overflow : "hidden"
				})
			}
...
(function(b) {
	...
	var c = {
		...
		className : "jslider",
		selector : ".jslider-",
		template : jSliderTmpl('<span class="<%=className%>"><table><tr><td><div class="<%=className%>-bg"><i class="l"><i></i></i><i class="r"><i></i></i><i class="v"><i></i></i></div><div class="<%=className%>-pointer" onmousedown="mouseDownOperate()"><i></i></div><div class="<%=className%>-pointer <%=className%>-pointer-to"><i></i></div></td></tr></table></span>')
	};
	...
	jSlider.prototype = {
		init : function(e, d) {
			...
			if (this.settings.calculate
					&& b.isFunction(this.settings.calculate)) {
				this.nice = this.settings.calculate
			}
			if (this.settings.onstatechange
					&& b.isFunction(this.settings.onstatechange)) {
		...
		create : function() {
			...
			b.extend(this.o, {
				...
				labels : {
					0 : {
						o : this.domNode.find(c.selector + "value").not(
								c.selector + "value-to")
					},
					1 : {
						o : this.domNode.find(c.selector + "value").filter(
								c.selector + "value-to")
					}
				},

				limits : {
					0 : this.domNode.find(c.selector + "label").not(
							c.selector + "label-to"),
					1 : this.domNode.find(c.selector + "label").filter(
							c.selector + "label-to")
				}
			});
		...
		redrawLabels : function(j) {
			...
			if (!this.settings.single) {
				...
				switch (j.uid) {
				case 0:
					if (h.border + h.label / 2 > i.o.offset().left
						...
						if (d.value.prc != j.value.prc) {
							g.value.html(this.nice(j.value.origin)
									+ "&nbsp;&ndash;&nbsp;"
									+ this.nice(d.value.origin));
				...
				case 1:
					if (h.border - h.label / 2 < i.o.offset().left
							- this.sizes.domOffset.left + i.o.outerWidth()) {
						i.o.css( {
							visibility : "hidden"
						});
						i.value.html(this.nice(d.value.origin));
						...
						if (d.value.prc != j.value.prc) {
							g.value.html(this.nice(d.value.origin)
									+ "&nbsp;&ndash;&nbsp;"
									+ this.nice(j.value.origin));
							h.label = g.o.outerWidth();
							h.border = (k * this.sizes.domWidth) / 100
						}
					} else {
			...
			if (this.settings.limits) {
				...
				for (key in this.o.pointers) {
					if (!this.settings.single || key == 0) {
						...
						if (h + e.o.outerWidth() > this.sizes.domWidth
								- d.outerWidth()) {
							f[1] = false
						}
					}
		...
		prcToValue : function(l) {
			if (this.settings.heterogeneity
					&& this.settings.heterogeneity.length > 0) {
		...
		valueToPrc : function(j, l) {
			if (this.settings.heterogeneity
				...
				var m = l.limits((j - this.settings.from) * 100
		...
		round : function(d) {
			...
			if (this.settings.round) {
				d = Math.round(d * Math.pow(10, this.settings.round))
		...
		nice : function(d) {
			...
			if (Number.prototype.jSliderNice) {
				return (new Number(d)).jSliderNice(this.settings.round)
						.replace(/-/gi, "&minus;")
			} else {
				return new Number(d)
			}
	...
	a.inheritFrom(Draggable, {
		...
		onmouseup : function(d) {
			if (this.parent.settings.callback
					&& b.isFunction(this.parent.settings.callback)) {
				this.parent.settings.callback.call(this.parent, this.parent
						.getValue())
			}
		...
		calc : function(e) {
			var d = this.limits(((e - this._parent.offset.left) * 100)
					/ this._parent.width);
			return d
		},
		set : function(d, e) {
